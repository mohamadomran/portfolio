  ...
  if (body._type === 'post') {
    ...
    if (!exists) {
      ...
      const moreStories = await client.fetch(
        groq`count(
          *[_type == "blog"] | order(date desc, _updatedAt desc) [0...3] [dateTime(date) > dateTime($date)]
        )`,
        { date: body.date },
      );
...
async function _queryAllRoutes(client: SanityClient): Promise<string[]> {
  return await client.fetch(groq`*[_type == "blog"].slug.current`);
}
...
async function mergeWithMoreStories(
  ...
  const moreStories = await client.fetch(
    groq`*[_type == "blog"] | order(date desc, _updatedAt desc) [0...3].slug.current`,
  );
...
async function queryStalePostRoutes(
  ...
  let slugs = await client.fetch(
    groq`*[_type == "blog" && _id == $id].slug.current`,
    { id },
  );
